...
public class AssetDataService : IAssetDataService
{
    ...
    public async Task CreateUpdateInternal(InternalAsset asset, bool onlyDetail = false)
    {
        ...
        await _sqlQuery.DynamicExecute("sp_InsertUpdateAsset", dynamicParameters, CommandType.StoredProcedure);
    }
    ...
    public async Task<IEnumerable<object>> GetInternalDynamics(string assetCode = "", string categoryIds = "", string subCategory = "", string brand = "", string companyCode = "", string status = "", string fields = "")
    {
    ...
    public async Task<ExternalAsset> GetExternalAsset(string assetCode)
    {
        ...
        var results = await _sqlQuery.DynamicQuery<ExternalAsset>(strSQL,
               new { assetType = "external", assetCode });
